...
* `ltex.languageSettings.<LANGUAGE>.disabledRules`: List of rules that should be disabled (if enabled by default by LanguageTool).
  * Example: `["EN_QUOTES", "UPPERCASE_SENTENCE_START"]` (for `en-US`); default: `[]`

* `ltex.languageSettings.<LANGUAGE>.enabledRules`: List of rules that should be enabled (if disabled by default by LanguageTool).
